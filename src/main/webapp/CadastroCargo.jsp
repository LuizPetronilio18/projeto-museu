...
<body>
    ...
    <form action="${pageContext.request.contextPath}${URL_BASE}/CargoControlador" method="GET">
        <input type="hidden" name="codCargo" value="${not empty codCargo ? codCargo : '0'}"/>
        <input type="hidden" name="opcao" value="${not empty opcao ? opcao : 'cadastrar'}"/>

        <fieldset>
            <legend>${not empty codCargo and codCargo != '0' ? 'Editar Cargo' : 'Novo Cargo'}</legend>

            <label for="nomeCargo">Nome do Cargo:</label><br>
            <input type="text" name="nomeCargo" id="nomeCargo" value="${nomeCargo}" required/><br>

            <label for="turno">Turno:</label><br>
            <input type="text" name="turno" id="turno" value="${turno}" required/><br>

            <label for="salario">Salário (R$):</label><br>
            <input type="number" step="0.01" name="salario" id="salario" value="${salario}" required/><br>
    ...
    <form action="${pageContext.request.contextPath}${URL_BASE}/CargoControlador" method="get" target="_blank" style="margin-bottom: 20px;">
        ...
        <tbody>
            <c:forEach var="cargo" items="${listaCargo}">
                <tr>
                    ...
                    <td>${cargo.turno}</td>
                    <td><fmt:formatNumber value="${cargo.salario}" type="currency" currencySymbol="R$ "/></td>
                    <td><a href="${pageContext.request.contextPath}${URL_BASE}/CargoControlador?opcao=editar&codCargo=${cargo.codCargo}&nomeCargo=${cargo.nomeCargo}&turno=${cargo.turno}&salario=${cargo.salario}">Editar</a></td>
                    <td><a href="${pageContext.request.contextPath}${URL_BASE}/CargoControlador?opcao=excluir&codCargo=${cargo.codCargo}&nomeCargo=${cargo.nomeCargo}&turno=${cargo.turno}&salario=${cargo.salario}">Excluir</a></td>
                </tr>
            </c:forEach>
        </tbody>
    </table>
