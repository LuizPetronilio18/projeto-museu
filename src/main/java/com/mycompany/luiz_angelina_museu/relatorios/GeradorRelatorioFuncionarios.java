...
public class GeradorRelatorioFuncionarios {

    // Busca dados para o gráfico: contagem de funcionários por cargo
    private JRMapCollectionDataSource getDadosGrafico() {
        String sql = "SELECT c.nomeCargo, COUNT(f.codFuncionario) AS total_funcionarios " +
                     "FROM funcionarios f " +
        ...
        try (Connection conexao = ConnectionFactory.getInstance().getConnection();
            ...
            while (rs.next()) {
                ...
                item.put("TOTAL_FUNCIONARIOS", rs.getInt("total_funcionarios"));
    ...
    public void geraRelatorio(HttpServletResponse response) throws IOException {
        try (Connection conexao = ConnectionFactory.getInstance().getConnection()) {
            InputStream relatorioStream = getClass().getResourceAsStream("/relatorio_funcionarios.jrxml");

            Map<String, Object> parametros = new HashMap<>();
            parametros.put("DADOS_GRAFICO_FUNCIONARIOS", getDadosGrafico());
            ...
            response.setHeader("Content-disposition", "inline; filename=relatorio_funcionarios.pdf");
