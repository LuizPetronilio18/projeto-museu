...
public class GeradorRelatorioCargos {

    // Busca dados para o gráfico: Custo total (salário * n_funcionarios) por cargo
    private JRMapCollectionDataSource getDadosGrafico() {
        String sql = "SELECT c.nomeCargo, (c.salario * COUNT(f.codFuncionario)) AS custo_total " +
                     "FROM cargo c " +
                     "LEFT JOIN funcionarios f ON c.codCargo = f.cargo_codCargo " +
        ...
        try (Connection conexao = ConnectionFactory.getInstance().getConnection();
            ...
            while (rs.next()) {
                ...
                item.put("NOME_CARGO", rs.getString("nomeCargo"));
                item.put("CUSTO_TOTAL", rs.getDouble("custo_total"));
    ...
    public void geraRelatorio(HttpServletResponse response) throws IOException {
        try (Connection conexao = ConnectionFactory.getInstance().getConnection()) {
            InputStream relatorioStream = getClass().getResourceAsStream("/relatorio_cargos.jrxml");

            Map<String, Object> parametros = new HashMap<>();
            parametros.put("DADOS_GRAFICO_CARGOS", getDadosGrafico());
            ...
            response.setHeader("Content-disposition", "inline; filename=relatorio_cargos.pdf");
            ...
            e.printStackTrace();
            throw new IOException("Erro ao gerar o relatório de cargos.", e);
        }
    }
}