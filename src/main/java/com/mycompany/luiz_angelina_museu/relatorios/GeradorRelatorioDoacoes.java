...
public class GeradorRelatorioDoacoes {

    // Busca os dados para o gráfico: total doado por museu
    private JRMapCollectionDataSource getDadosGrafico() {
        String sql = "SELECT m.nome, SUM(d.valorDoacao) as total " +
                     "FROM doacoes d " +
                     "JOIN museus m ON d.Museus_codMuseu = m.codMuseu " +
                     "GROUP BY m.nome " +
                     "ORDER BY total DESC";
        ...
        try (Connection conexao = ConnectionFactory.getInstance().getConnection();
            ...
            while (rs.next()) {
                ...
                item.put("TOTAL_DOADO", rs.getDouble("total"));
    ...
    public void geraRelatorio(HttpServletResponse response) throws IOException {
        try (Connection conexao = ConnectionFactory.getInstance().getConnection()) {
            InputStream relatorioStream = getClass().getResourceAsStream("/relatorio_doacoes.jrxml");

            Map<String, Object> parametros = new HashMap<>();
            parametros.put("DADOS_GRAFICO_DOACOES", getDadosGrafico());
            ...
            response.setHeader("Content-disposition", "inline; filename=relatorio_doacoes.pdf");
            ...
            e.printStackTrace();
            throw new IOException("Erro ao gerar o relatório de doações.", e);
        }
    }
}