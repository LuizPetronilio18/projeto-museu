...
public class GeradorRelatorioArtistas {

    private JRMapCollectionDataSource getDadosGrafico() {
        String sql = "SELECT nacionalidadae, COUNT(codArtista) AS total FROM artistas GROUP BY nacionalidadae";
        ...
        try (Connection conexao = ConnectionFactory.getInstance().getConnection();
            ...
            while (rs.next()) {
                ...
                item.put("NACIONALIDADE", rs.getString("nacionalidadae"));
    ...
    public void geraRelatorio(HttpServletResponse response) throws IOException {
        try (Connection conexao = ConnectionFactory.getInstance().getConnection()) {
            InputStream relatorioStream = getClass().getResourceAsStream("/relatorio_artistas.jrxml");

            Map<String, Object> parametros = new HashMap<>();
            parametros.put("DADOS_GRAFICO_NACIONALIDADE", getDadosGrafico());
            ...
            response.setHeader("Content-disposition", "inline; filename=relatorio_artistas.pdf");
            ...
            e.printStackTrace();
            throw new IOException("Erro ao gerar o relatório de artistas.", e);
        }
    }
}